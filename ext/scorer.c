...
int t2_per_round[6] = {1,2,4,8,16,32};

void t2_set_round_factor(int round, int value) {
  if (round <= 5) {
    t2_per_round[round] = value;
  }
}

int t2_games_in_round(int n_teams, int r) {
  return (n_teams / 2) / (2ULL << r);
}

int t2_basic(int round, int winner, int loser) {
  static int roundScores[6] = {1,2,4,8,16,32};
  return t2_per_round[round];
}
...
int t2_josh_patashnik(int round, int winner, int loser) {
  ...
  return t2_per_round[round] * winnerTeam.seed;
}

int t2_seed_difference(int round, int winner, int loser) {
  static int multipliers[6] = {1,2,4,8,16,32};
  TeamObj winnerTeam = NCAA_TEAMS[winner];
  TeamObj loserTeam = NCAA_TEAMS[winner];
  int diff = winnerTeam.seed - loserTeam.seed;
  if (diff <= 0) {
    diff = 1;
  }
  return t2_per_round[round] * diff;
}

int t2_constant_value(int round, int winner, int loser) {
  return t2_per_round[0];
}
...
scoringMethodPtr t2_scoring_method_for(enum ScoringMethod scoringMethod) {
  switch(scoringMethod) {
    ...
    case SEED_DIFFERENCE:
      return t2_seed_difference;
  }
}
